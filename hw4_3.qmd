...
title: "HW4"
date: today
author: 第六組(林智達_H24095031_江泰佳_H24106086_陳凱騫 H24101222)
format:
 pdf:
    include-in-header:
      - text: |
...
將"遺漏值或跳答"改為遺漏值，並以回答數(v4的總和)作為政治熱衷程度(以新變
量"v4"代表)。
v2與v3可以合併為一個變數，因為都是區底下的里，且不會出現交集。

```{r}
data[data=="遺漏值或跳答"] <- NA
v4 <- rowSums(!is.na(subset(data,select=c(v4_1,v4_2,v4_3,v4_4,v4_5,v4_6,v4_7,v4_8))))
data_new <- cbind(subset(data,select=-c(v4_1,v4_2,v4_3,v4_4,v4_5,v4_6,v4_7,v4_8)),v4)
v9 <- coalesce(data_new$v2, data_new$v3)
data_new <- cbind(subset(data_new,select=-c(v2,v3)),v9)
data_new <- data_new %>%
  select(v1,v9,v4,v5,v6,v7,v8)
# head(data_new,n=20)
```
...
table1(~.|v1,data_new)
```
遺失值，先定義每一個變量的遺失定義(使用summary/table找出可能的遺失選項)。

```{r}
discrip <- data.frame(variable=c("v1(區)、v4(熱衷度)、v8(性別)","v9(里)","v5(支持)",
                                 "v6(年紀)","v7(學歷)"),
                      discription=c("不會出現遺失值","不知道、拒答","無反應、尚未決定、91",
                                    "不知道╱拒答","拒答"))
  #table(data_new$v5)
kable(discrip,caption="遺失選項")
data_new[data_new$v9=="不知道"|data_new$v9=="拒答",]$v9 <- NA
data_new[data_new$v5=="無反應"|data_new$v5=="尚未決定"|data_new$v5==91,]$v5 <- NA
data_new[data_new$v6=="不知道╱拒答",]$v6<- NA
data_new[data_new$v7=="拒答",]$v7 <- NA
...
```{r}
data_nom <- data_new[!is.na(data_new$v5),]
table1(~.|v5,data_nom)
```
以直條圖表示支持人數與支持率。
...
```{r}
data5 <- data_new
data5[is.na(data5$v5),] <- "遺失"
barplot(table(data5$v5),xlab="候選人",ylab="支持的人數",main = "各候選人支持人數直條圖",ylim = c(0,220))
  #總人數
num <- length(data_nom$v5)
barplot(table(data_nom$v5)/num,ylim=c(0,0.3),xlab="候選人",ylab="支持比率",
        main="各候選人支持比率直條圖")
```
## 第三題
...
library(sf)
#讀地圖檔
map <- st_read("VILLAGE_NLSC_1130807.shp")
map1 <- map[map$COUNTYNAME=="臺南市",]
map1 <- map1[map1$TOWNNAME=="北區"|map1$TOWNNAME=="中西區",]
```
...
map1 <- merge(map1,num,by="VILLNAME",all=T)
map1[is.na(map1)] <- 0
```
...
library(ggplot2)
describe(map1$Freq)
map_N <- map1[map1$TOWNNAME=="北區",]
map_MW <- map1[map1$TOWNNAME=="中西區",]
```
由兩區地圖可以看出，三號候選人在西北方有較高的支持度，尤其是在大港里。在東南部分支持者較少。

```{r}
#繪製兩區地圖
coord <- as.data.frame(st_centroid(map1))
f <- st_coordinates(st_centroid(map1))
ggplot(data = map1) +
