...
$$
P_{t} = P^{*}_{t} S_{t} \rightarrow q_t = s_t + p^*_{t}-p_t = 0
$$
...
#| code-fold: FALSE
# Johansen 共整合檢定
jotest.t <- ca.jo(data.frame(ewaAdj, ewcAdj, igeAdj), type="trace", K=4, ecdet="none", spec="longrun")
summary(jotest.t)
```
...
```{r}
#| code-fold: FALSE
jotest.m <- ca.jo(data.frame(ewaAdj, ewcAdj, igeAdj), type="eigen", K=4, ecdet="none", spec="longrun")
summary(jotest.m)
```
...
# 計算共整合時間序列（CI）
CI <- ewaAdj+ alpha1 * ewcAdj+ alpha2 * igeAdj
...
#| code-fold: true 
# ADF檢定共整合時間序列
# 提取第一個共整合向量 (對應於最多一個共整合關係)
cointegration_vector <- jotest.t@V[, 1]  # 提取第一個共整合向量
# 計算殘差 (共整合關係)
residuals <- as.matrix(data.frame(ewaAdj, ewcAdj, igeAdj)) %*% cointegration_vector
...
- 最大特徵值檢定更集中於某一個特徵值的增量效果，因此可能更敏感於檢測出一個特定的共整合關係。
...
adf.test(r)
library(vars)
var_select <- VARselect(data.frame(p, q, r), lag.max = 10, type = "none")
jotest=ca.jo(data.frame(p,q,r), type="trace", K=4, ecdet="none", spec="longrun")
```
...
```{r}
#| code-fold: false
s = 1.000*p  -1.843261*q + 4.017197*r
plot(s, type="l")
library("tseries")
adf.test(s)
```
...
- [https://github.com/KaiChienChen/Statistical-Consulting/blob/main/midterm_presentation.qmd](https://github.com/KaiChienChen/Statistical-Consulting/blob/main/midterm_presentation.qmd)
